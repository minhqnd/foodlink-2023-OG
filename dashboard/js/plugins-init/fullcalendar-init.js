$.getJSON('https://order-app-moi-default-rtdb.asia-southeast1.firebasedatabase.app/book.json', function (data) {
    var cc = []
    console.log(data)
    for (item in data) {
        cc.push(data[item]);
    ...
    const result = [];
    cc.forEach(item => {
        const guestsText = `Bàn ${item.guests} người - ${item.phone}`;
        const formattedTime = moment(item.time).valueOf();
        result.push({
            title: guestsText,
            start: formattedTime,
            className: "bg-dark"
        ...
        t.prototype.onDrop = function (t, n) {
            ...
            i.append("<div class='row'></div>"), i.find(".row").append("<div class='col-md-6'><div class='form-group'><label class='control-label'>Tên sự kiện</label><input class='form-control' placeholder='Nhập tên sự kiện' type='text' name='title'/></div></div>").append("<div class='col-md-6'><div class='form-group'><label class='control-label'>Danh mục</label><select class='form-control' name='category'></select></div></div>").find("select[name='category']").append("<option value='bg-danger'>Khẩn cấp</option>").append("<option value='bg-success'>Ưu  tiên</option>").append("<option value='bg-dark'>Dark</option>").append("<option value='bg-primary'>Primary</option>").append("<option value='bg-pink'>Pink</option>").append("<option value='bg-info'>Info</option>").append("<option value='bg-warning'>Warning</option></div></div>"), o.$modal.find(".delete-event").hide().end().find(".save-event").show().end().find(".modal-body").empty().prepend(i).end().find(".save-event").unbind("click").on("click", function () {
                ...
                return null !== e && 0 != e.length ? (o.$calendarObj.fullCalendar("renderEvent", {
                    ...
                    allDay: !1,
                    className: a
                }, !0), o.$modal.modal("hide")) : alert("Cần tiêu đề cho sự kiện"), !1
